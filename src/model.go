...
func (s *Storage) FetchSummit(summitId string, userId int64) (*Summit, error) {
	...
	if err := s.db.Pool.QueryRow("SELECT summit_id FROM summit_ids_legacy WHERE legacy_id = ?", summitId).Scan(&canonicalId); err != nil {
		if err != sql.ErrNoRows { // любая иная ошибка
			return nil, err
		}
		// sql.ErrNoRows означает, что это не legacy — используем переданный ID напрямую
	}
	// Выполняем основной запрос по canonicalId (может вернуть sql.ErrNoRows если вершины нет вовсе)
